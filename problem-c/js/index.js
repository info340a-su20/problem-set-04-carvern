...
var huskyOpponents = huskyGames2016.map(extractOpponent);
console.log(huskyOpponents);
...
var huskyLoss = huskyGames2016.filter(huskiesLost);
console.log(huskyLoss);


//Log out an array of opponents that UW lost to. Hint: Use the `.map()` method 
//to extract the opponent names!

var huskyOpponentsLost = huskyLoss.map(extractOpponent);
console.log(huskyOpponentsLost);
...
huskyLoss.forEach(function(x) { 
    console.log(x["opponent"] + " at " + x["home"] + ", " + x["opponent_score"] + " to " + x["home_score"]);
  }
);
...
huskyGames2016.filter(function(x) {
  if(x["fumbles"] > 0) {
    fumbles.push(x["fumbles"]);
  }
});
...
var lostFumbles = huskyGames2016.filter(fumbledAndLostFilter);
